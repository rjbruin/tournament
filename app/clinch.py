...
Only top-two clinches are reported as theoretical advancement: the top two of
each group always advance, and that is decidable from the group alone. Third
place may or may not advance depending on the other groups' best-third race, so
that case is intentionally left to the statistical (">99.9%") path.
"""
...
def advances_for_sure(status: str | None) -> bool:
    ...
    return status in (CLINCHED_FIRST, CLINCHED_TOP2)
